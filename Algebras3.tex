...
\end{haskell}
A value of this type is a polymorphic function. This function, for any type \hask{a} and any function \hask{f a -> a}, will produce a value of type \hask{a}. In other words, it's a gigantic product of all possible catamorphisms. 
...
\end{haskell}
In essence, if we can provide the implementation of all possible folds for a given list, we have defined the list. This is like defining a function by specifying its integrals with all possible test functions (that's how distributions are defined in calculus).
\begin{haskell}
...
\end{haskell}
We can now apply the standard trick: a function from a sum type is equivalent to a product of functions. Here we have a gigantic product of functions, so we need a gigantic sum. A gigantic sum is known as the existential type. So, symbolically, in pseudo Haskell, we would define \hask{Nu} as
...
\end{haskell}
This literally does nothing, just stores a coalgebra and a seed. But that's how existential types work: they can be instantiated with a single value. The whole logic is in the deconstruction, or the usage of this value. 
...
data StreamF a x = StreamF a x
  deriving Functor
\end{haskell}
An infinite stream of \hask{a} is the initial algebra for this functor
\begin{haskell}
...
\end{haskell}
We can use it to generate a stream of natural numbers. We'll express it using \hask{Nu}
\begin{haskell}
...
We know from the previous installment that there is a canonical mapping (an injection) from \hask{Mu f} to \hask{Nu f}. It turns out that \hask{fromNuToMu} is its inverse, proving that, in Haskell, these two fixed points are isomorphic.
...
\end{haskell}
This is an example of a hylomorphism. The actual implementation of a hylomorphism in the Haskell library is more efficient, as it fuses the algebra and coalgebra directly
\begin{haskell}
...
which has the form reminiscent of the Haskell hylomorphism
